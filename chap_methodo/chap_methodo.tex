...
Similarly, the salvage value is computed in the proportion of the remaining years of life versus the initial lifetime of an installed capacity of a technology from which the anticipatively decommissioned part, $\textbf{F\textsubscript{decom}}$, is removed. After assessing the sensitivity of the formulation of the salvage value (see Appendix \ref{sec:path_methodo}), we have kept the one given by Equation (\ref{eq:salvage}), being also similar to the expression used by \citet{prina2019transition}.
...
Section \ref{sec:cs:uncertainty} gathers the list of the uncertain parameters considered in this work.
